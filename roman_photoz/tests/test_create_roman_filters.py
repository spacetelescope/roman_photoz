...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {},
            {"url": BASE_URL.format("20220101"), "dest": Path("test_Roman_effarea_20220101.xlsx").resolve().as_posix()},
        ),
    ],
...
def test_read_effarea_file(file_exists, test_file, expected_kwargs, expected_download):
    ...
    with patch("pathlib.Path.exists", return_value=file_exists) as mock_exists:
        ...
        if file_exists:
            ...
            with patch("roman_photoz.create_roman_filters.download_file") as mock_download, \
                ...
                mock_download.assert_called_once_with(
                    expected_download["url"], 
                    expected_download["dest"]
                )
...
def test_create_files(sample_dataframe):
    ...
    with patch("roman_photoz.create_roman_filters.create_path", return_value=test_path) as mock_create_path, \
         patch("builtins.open", MagicMock()) as mock_open, \
         patch("roman_photoz.create_roman_filters.create_roman_phot_par_file") as mock_create_par:
        ...
        assert mock_open.call_count == 2  # One for each filter column (excluding wavelength)
...
def test_run_filter_command():
    ...
    with patch("roman_photoz.create_roman_filters.create_path", return_value=Path("/test/path")) as mock_create_path, \
...
def test_run():
    ...
    with patch("roman_photoz.create_roman_filters.read_effarea_file") as mock_read, \
         ...
         patch("roman_photoz.create_roman_filters.run_filter_command") as mock_run_filter:
        ...
        mock_create_files.assert_called_once_with(data=mock_df, filepath="/test/output/path")
