...
def test_njy_to_mgy_scalar():
    ...
    flux_njy = 3631e9 * u.Unit("nJy")  # 1 maggy
...
def test_njy_to_mgy_array():
    ...
    flux_njy = np.array([3631e9, 7262e9]) * u.Unit("nJy")
    flux_mgy = njy_to_mgy(flux_njy)
    assert np.allclose(flux_mgy.value, [1.0, 2.0])


def test_update_fluxes(monkeypatch):
    """Test update_fluxes updates flux columns and copies label/redshift columns, without fudge_factor."""
    ...
    expected_f184 = njy_to_mgy(flux["segment_f184_flux"]).value * scaling_factor
    np.testing.assert_allclose(np.asarray(updated_scaled["F184"]), expected_f184)
    # F213 should still be target
    np.testing.assert_allclose(
        np.asarray(updated_scaled["F213"]), np.asarray(target["F213"])
    )
