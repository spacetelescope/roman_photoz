...
class TestRomanCatalogProcess:
    ...
    @pytest.mark.parametrize("file_exists, expected", [
        (True, True),
        (False, False)
    ])
    ...
    @pytest.mark.parametrize("model_exists, should_create_informer", [
        (False, True),   # When model doesn't exist, create_informer_stage should be called
        (True, False)    # When model exists, create_informer_stage should NOT be called
    ])
    @patch('os.path.exists')
    @patch('roman_photoz.roman_catalog_process.RomanCatalogProcess.create_informer_stage')
    @patch('roman_photoz.roman_catalog_process.RomanCatalogProcess.create_estimator_stage')
    def test_process_model_existence(self, mock_create_estimator, mock_create_informer, 
                                    mock_exists, model_exists, should_create_informer):
